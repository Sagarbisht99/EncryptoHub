  ...
  return (
    <>
      {variant === "list" ? (
        ...
        <div className={cn("rounded-xl p-4 shadow-md transition-all duration-300 border", cardBg)}>
          <div className="flex items-start gap-4">
            <div className={`w-12 h-12 ${theme === "dark" ? "bg-zinc-700" : "bg-gray-100"} rounded-md flex items-center justify-center text-2xl`}>
              📄
            </div>
            <div className="flex-1">
              <h3 className={`font-medium text-base ${textColor}`}>{file.title}</h3>
              <p className={`text-xs mt-1 ${subTextColor}`}>{getTimeAgo(file.createdAt)}</p>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <button
              onClick={() => setShareModalOpen(true)}
              className="px-3 py-1.5 rounded-full bg-green-100 dark:bg-green-800 dark:text-white text-green-700 hover:bg-green-200 dark:hover:bg-green-700 transition text-sm"
              title="Share"
            >
              Share
            </button>
            <button
              onClick={() => openRenameModal(file._id, file.title)}
              className="px-3 py-1.5 rounded-full bg-indigo-100 dark:bg-indigo-800 dark:text-white text-indigo-700 hover:bg-indigo-200 dark:hover:bg-indigo-700 transition text-sm"
              title="Rename"
            >
              Rename
            </button>
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="px-3 py-1.5 rounded-full bg-blue-100 dark:bg-blue-800 text-blue-700 dark:text-white hover:bg-blue-200 dark:hover:bg-blue-700 transition text-sm"
              title="View"
            >
              View
            </Link>
            <button
              onClick={() => openDeleteModal(file._id)}
              className="px-3 py-1.5 rounded-full bg-red-100 dark:bg-red-800 text-red-700 dark:text-white hover:bg-red-200 dark:hover:bg-red-700 transition text-sm"
              title="Delete"
            >
              Delete
            </button>
